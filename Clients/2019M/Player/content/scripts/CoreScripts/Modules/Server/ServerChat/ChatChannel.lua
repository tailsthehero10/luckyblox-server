...
function methods:SendMessageToSpeaker(message, speakerName, fromSpeakerName, extraData)
	...
	if speakerTo and speakerFrom then
		...
		local isFiltered = true--speakerName == fromSpeakerName
		...
		--// OLD BEHAVIOR
		local filteredMessage = message--self.ChatService:InternalApplyRobloxFilter(messageObj.FromSpeaker, message, speakerName)
